...
class SoundSearchTile @Inject constructor(
    ...
    override fun handleClick(@Nullable view: View?) {
        ...
        // Shazam
        if (systemUtils.isPackageInstalled(context, "com.shazam.android") ||
            systemUtils.isPackageInstalled(context, "com.shazam.encore.android")
        ) {
        ...
        // Soundhound
        else if (systemUtils.isPackageInstalled(
                context,
                "com.melodis.midomiMusicIdentifier.freemium"
            ) || systemUtils.isPackageInstalled(context, "com.melodis.midomiMusicIdentifier")
        ) {
            val intent = Intent(Intent.ACTION_MAIN)
            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TASK)
